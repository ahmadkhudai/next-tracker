...
export function AddExpenseForm({addNewExpense}:Props) {
    ...
    const defaultExpense = {name: "chai", price: 30, description: "Chai from Khana Pena. Very Good👍", date: currentlySelectedDate};
                ...
                {/*{dateSelector!==dateSelectors.custom &&*/}
                    <div className={"flex justify-content-between pb-3"}>
                        ...
                        {dateSelector===dateSelectors.thisWeek &&
                            <div className={" flex flex-column overflow-x-scroll justify-content-between scrollable px-3 "}>
                            {getSubtractableDays(moment(new Date()).weekday()).map
                            (
                                day =>{

                                    return <button key={day} className={"btn bg-purple-400 hover:font-bold hover:bg-teal-400 text-white my-1"} onClick={()=>{handleFieldChange("date", addDays(new Date(), -(day)));console.log(day);}}>{Day[moment(new Date()).weekday()-day]}</button>
